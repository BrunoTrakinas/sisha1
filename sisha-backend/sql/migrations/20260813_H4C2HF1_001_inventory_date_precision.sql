-- SISHA1 V2 - H4C2-HF1
-- Correcao da data/hora de inventario serializado.
--
-- Regra:
-- * data_snapshot e uma DATA efetiva (sem hora);
-- * data_evento usa a meia-noite da data civil no fuso America/Sao_Paulo;
-- * created_at/registrado_em preservam a hora real em que o SISHA registrou o evento;
-- * nenhum horario efetivo e inventado na interface.
--
-- Inclui reparo auditavel apenas dos eventos antigos que correspondem
-- exatamente ao cast defeituoso conhecido date -> timestamptz em UTC.
...
declare
  ...
  v_count integer := 0;
  v_total integer;
  -- O inventario fornece uma DATA civil efetiva, sem hora.
  -- Usamos a meia-noite dessa data no fuso operacional do SISHA para a
  -- ordenacao tecnica, sem deslocar a data para o dia anterior.
  v_snapshot_event_at timestamptz := p_snapshot_date::timestamp AT TIME ZONE 'America/Sao_Paulo';
...
begin
  ...
  loop
    ...
    insert into public.equipamento_eventos (
      ...
      v_equipment_id, v_pn, v_sn, 'INVENTARIO_EQUIPAMENTOS', v_snapshot_event_at,
      ...
      jsonb_build_object(
        ...
        'precisao_temporal', 'DATA',
        'fuso_operacional', 'America/Sao_Paulo',
        'registrado_em', v_registered_at,
...
-- Preserva o hardening H4C2.
revoke execute on function public.sisha_apply_equipment_inventory_import(text,date,text,text,text,jsonb) from public;
revoke execute on function public.sisha_apply_equipment_inventory_import(text,date,text,text,text,jsonb) from anon;
revoke execute on function public.sisha_apply_equipment_inventory_import(text,date,text,text,text,jsonb) from authenticated;
grant execute on function public.sisha_apply_equipment_inventory_import(text,date,text,text,text,jsonb) to service_role;

-- Reparo cirurgico do bug legado.
-- O timestamp original fica preservado no payload.
update public.equipamento_eventos e
set
  payload = coalesce(e.payload, '{}'::jsonb) || jsonb_build_object(
    'precisao_temporal', 'DATA',
    'fuso_operacional', 'America/Sao_Paulo',
    'timezone_repair', 'H4C2HF1',
    'data_evento_original_utc', e.data_evento,
    'registrado_em', coalesce(e.created_at, now())
  ),
  data_evento = ((e.payload->>'data_snapshot')::date::timestamp AT TIME ZONE 'America/Sao_Paulo')
where e.tipo_evento = 'INVENTARIO_EQUIPAMENTOS'
  and e.invalidado is distinct from true
  and e.payload ? 'data_snapshot'
  and (e.payload->>'data_snapshot') ~ '^\d{4}-\d{2}-\d{2}$'
  and e.data_evento = ((e.payload->>'data_snapshot')::date::timestamptz);

insert into public.sisha_schema_migrations(version, description)
values (
  '20260813_H4C2HF1_001',
  'H4C2-HF1: corrige data civil do inventario e separa data efetiva da hora real de registro.'
)
...
begin
  ...
  if v_anon or v_auth or not v_service then
    raise exception 'H4C2-HF1: privilegios da RPC de inventario ficaram incorretos.';
  end if;
end;
$verify$;
