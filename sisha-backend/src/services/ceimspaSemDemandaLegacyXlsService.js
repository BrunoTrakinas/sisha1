const { parseLegacyXlsRows } = require('./ceimspaSemDemandaLegacyXlsReader');
...
function buildIndex(headers = []) {
  ...
  return {
    pi: first('PI'), nomePort: first('NOME_PORT'), nomeCol: first('NOME_COL'), ref: first('REF'), rnvc: first('RNVC'),
    om: first('OM'), meio: first('MEIO'), nome: first('NOME'), qtdApl: first('QTDE_APL'), qtdDot: first('QTDE_DOT'),
    cam: first('CAM'), qtdExistente: first('QTDE_EXISTENTE'), qtdDisponivel: first('QTDE_DISPONIVEL'),
  };
}
function cell(row, index) { return index >= 0 ? row[index] : null; }

function parseCeimspaSemDemandaLegacyXls(filePath, options = {}) {
  let headerRow = -1;
  let idx = null;
  ...
  const meta = parseLegacyXlsRows(filePath, (rowIndex, row) => {
    if (headerRow < 0) {
      if (rowIndex > 100) return;
      const headers = row.map(normalizeHeader);
      if (headers.includes('PI') && headers.includes('REF') && headers.includes('QTDE_DISPONIVEL')) {
        headerRow = rowIndex;
        idx = buildIndex(row);
      }
      return;
    ...
    const quantidade = parseNumber(cell(row, idx.qtdDisponivel));
    ...
    if (Math.abs(group.quantidade - quantidade) > 1e-9 || Math.abs(group.quantidade_existente - quantidadeExistente) > 1e-9) {
  ...
  return { headerIndex: headerRow, sourceRows, validRows, rows, reader: meta };
}

module.exports = { parseCeimspaSemDemandaLegacyXls };
