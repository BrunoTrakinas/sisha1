  ...
  return (
    <section className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-black text-slate-800 uppercase">Base do Gerador de Necessidade • Fase 5A</h2>
          <p className="text-sm text-slate-700 mt-1">Use a Central de Inserção para importar XLSX de Receitas, PIM e Política. Aqui você pode pesquisar, corrigir e cadastrar manualmente.</p>
        </div>
        <div className="grid grid-cols-3 gap-3 min-w-[280px]">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-center">
            <p className="text-[11px] uppercase font-black text-slate-500">Itens de Receita</p>
            <p className="text-2xl font-black text-slate-900">{loadingSnapshot ? '...' : snapshot.receitaItens}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-center">
            <p className="text-[11px] uppercase font-black text-slate-500">PIMs</p>
            <p className="text-2xl font-black text-slate-900">{loadingSnapshot ? '...' : snapshot.pims}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-center">
            <p className="text-[11px] uppercase font-black text-slate-500">Políticas</p>
            <p className="text-2xl font-black text-slate-900">{loadingSnapshot ? '...' : snapshot.politicas}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-5 py-3 rounded-xl font-black ${activeTab === tab.key ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-900 hover:bg-slate-200'}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'receitas' && (
        <div className="grid grid-cols-1 xl:grid-cols-[340px_1fr] gap-6">
          <div className="space-y-4">
            <input value={receitaSearch} onChange={(e) => setReceitaSearch(e.target.value)} placeholder="Buscar inspeção/receita" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
            <div className="border border-slate-200 rounded-2xl overflow-auto max-h-[420px]">
              {receitas.map((item) => (
                <button key={item.inspecao} onClick={() => { setSelectedInspecao(item.inspecao); setReceitaForm({ ...emptyReceitaForm, inspecao: item.inspecao }); }} className={`w-full text-left p-4 border-b border-slate-200 last:border-b-0 ${selectedInspecao === item.inspecao ? 'bg-slate-900 text-white' : 'bg-white text-slate-900 hover:bg-slate-50'}`}>
          ...
          <div className="space-y-6">
            <div className="border border-slate-200 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <h3 className="text-lg font-black text-slate-900 uppercase">Itens da Receita</h3>
                <button onClick={() => setReceitaForm({ ...emptyReceitaForm, inspecao: selectedInspecao })} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-900 font-black hover:bg-slate-200">NOVO ITEM</button>
              </div>
              {selectedInspecao ? (
                <div className="overflow-auto border border-slate-200 rounded-2xl max-h-[320px]">
            ...
            <form onSubmit={salvarReceita} className="border border-slate-200 rounded-2xl p-5 space-y-4">
              <h3 className="text-lg font-black text-slate-900 uppercase">Cadastro Manual de Receita</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input value={receitaForm.inspecao} onChange={(e) => setReceitaForm((p) => ({ ...p, inspecao: e.target.value }))} placeholder="Inspeção / Receita" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500 md:col-span-2" />
                <input value={receitaForm.pn} onChange={(e) => setReceitaForm((p) => ({ ...p, pn: e.target.value.toUpperCase() }))} placeholder="PN" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
                <input value={receitaForm.nsn} onChange={(e) => setReceitaForm((p) => ({ ...p, nsn: e.target.value }))} placeholder="NSN" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
                <input value={receitaForm.pn_alt} onChange={(e) => setReceitaForm((p) => ({ ...p, pn_alt: e.target.value.toUpperCase() }))} placeholder="PN Alternativo (opcional)" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
                <input value={receitaForm.qtd_por_ciclo} onChange={(e) => setReceitaForm((p) => ({ ...p, qtd_por_ciclo: e.target.value }))} placeholder="Qtd por Ciclo" type="number" min="0" step="0.01" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
                <input value={receitaForm.nomenclatura} onChange={(e) => setReceitaForm((p) => ({ ...p, nomenclatura: e.target.value }))} placeholder="Nomenclatura" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500 md:col-span-2" />
              </div>
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setReceitaForm({ ...emptyReceitaForm, inspecao: selectedInspecao })} className="px-6 py-3 rounded-xl bg-slate-100 text-slate-900 font-black hover:bg-slate-200">LIMPAR</button>
      ...
      {activeTab === 'pims' && (
        <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.9fr] gap-6">
          <div className="space-y-4">
            <input value={pimSearch} onChange={(e) => setPimSearch(e.target.value)} placeholder="Buscar PIM, PN, OS, ANV ou Oficina" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
            <div className="overflow-auto border border-slate-200 rounded-2xl max-h-[520px]">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-100 text-slate-800 uppercase text-xs">
                  <tr>
          ...
          <form onSubmit={salvarPim} className="border border-slate-200 rounded-2xl p-5 space-y-4">
            <h3 className="text-lg font-black text-slate-900 uppercase">Cadastro Manual de PIM</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input value={pimForm.pim} onChange={(e) => setPimForm((p) => ({ ...p, pim: e.target.value }))} placeholder="PIM" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input value={pimForm.data_solicitacao} onChange={(e) => setPimForm((p) => ({ ...p, data_solicitacao: e.target.value }))} placeholder="Data Solicitação (AAAA-MM-DD)" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input value={pimForm.pn} onChange={(e) => setPimForm((p) => ({ ...p, pn: e.target.value.toUpperCase() }))} placeholder="PN" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input value={pimForm.nsn} onChange={(e) => setPimForm((p) => ({ ...p, nsn: e.target.value }))} placeholder="NSN" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input type="number" min="0" step="0.01" value={pimForm.quantidade} onChange={(e) => setPimForm((p) => ({ ...p, quantidade: e.target.value }))} placeholder="Quantidade" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input value={pimForm.os_vinculada} onChange={(e) => setPimForm((p) => ({ ...p, os_vinculada: e.target.value.toUpperCase() }))} placeholder="OS Vinculada" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input value={pimForm.observacoes} onChange={(e) => setPimForm((p) => ({ ...p, observacoes: e.target.value }))} placeholder="Observações" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500 md:col-span-2" />
            </div>
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setPimForm(emptyPimForm)} className="px-6 py-3 rounded-xl bg-slate-100 text-slate-900 font-black hover:bg-slate-200">LIMPAR</button>
      ...
      {activeTab === 'politicas' && (
        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
          <div className="space-y-4">
            <input value={politicaSearch} onChange={(e) => setPoliticaSearch(e.target.value)} placeholder="Buscar tarefa ou tipo" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
            <div className="overflow-auto border border-slate-200 rounded-2xl max-h-[520px]">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-100 text-slate-800 uppercase text-xs">
                  <tr>
          ...
          <form onSubmit={salvarPolitica} className="border border-slate-200 rounded-2xl p-5 space-y-4">
            <h3 className="text-lg font-black text-slate-900 uppercase">Cadastro Manual de Política</h3>
            <div className="grid grid-cols-1 gap-4">
              <input value={politicaForm.tarefas} onChange={(e) => setPoliticaForm((p) => ({ ...p, tarefas: e.target.value }))} placeholder="Tarefa" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <select value={politicaForm.tipo} onChange={(e) => setPoliticaForm((p) => ({ ...p, tipo: e.target.value }))} className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900">
                <option value="Receita">Receita</option>
                <option value="PIM">PIM</option>
                <option value="Demanda Avulsa">Demanda Avulsa</option>
              </select>
              <input type="number" min="0" step="1" value={politicaForm.prioridade} onChange={(e) => setPoliticaForm((p) => ({ ...p, prioridade: e.target.value }))} placeholder="Prioridade" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
              <input type="number" min="0" step="0.01" value={politicaForm.qtde_2_anos} onChange={(e) => setPoliticaForm((p) => ({ ...p, qtde_2_anos: e.target.value }))} placeholder="Qtde 2 anos" className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 placeholder:text-slate-500" />
            </div>
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setPoliticaForm(emptyPoliticaForm)} className="px-6 py-3 rounded-xl bg-slate-100 text-slate-900 font-black hover:bg-slate-200">LIMPAR</button>
