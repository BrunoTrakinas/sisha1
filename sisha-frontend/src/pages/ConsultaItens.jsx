...
export default function ConsultaItens() {
    ...
    const buscarItem = async (e) => {
        ...
        try {
            ...
            const json = await response.json();
            if (json.status === 'success') { setResultados(json.data); } else { setResultados([]); }
        } catch (error) { setResultados([]); }
        setCarregando(false);
    };

    return (
        <div className={`w-full h-full flex flex-col transition-all duration-700 ease-in-out ${buscaRealizada ? 'items-center pt-4' : 'items-center justify-center mt-[-10vh]'}`}>

            {!buscaRealizada && (
                <div className="text-center mb-8 animate-fade-in">
                    <h1 className="text-5xl font-black text-slate-800 tracking-tighter italic">RADAR <span className="text-blue-600">LOGÍSTICO</span></h1>
                    <p className="text-slate-400 font-bold mt-2 uppercase tracking-widest text-xs">SISHA-1 V2 • Motor de Inteligência Integrada</p>
                </div>
            )}

            <form onSubmit={buscarItem} className={`w-full px-4 transition-all duration-700 max-w-3xl ${buscaRealizada ? 'mb-6' : ''}`}>
                <div className="relative flex items-center shadow-md rounded-full bg-white border border-slate-200 focus-within:ring-4 focus-within:ring-blue-100 transition-all">
                    <div className="pl-5 text-blue-600"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div>
                    <input type="text" value={termo} onChange={(e) => setTermo(e.target.value)} placeholder="Pesquise por PN, Nome da peça ou SN..." className="w-full py-4 px-4 text-base bg-transparent border-none outline-none text-slate-700 uppercase" />
                    <button type="submit" disabled={carregando} className={`mr-2 bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-black hover:bg-blue-700 transition-all disabled:opacity-50 shadow-sm ${carregando ? 'animate-pulse' : ''}`}>
                        {carregando ? 'BUSCANDO...' : 'RADAR'}
                    </button>
                </div>
            </form>

            <div className="w-full max-w-3xl px-4 mb-4 flex justify-end">
                <button
                    type="button"
                    onClick={() => setModalLote(true)}
                    className="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-xl text-xs font-black hover:border-blue-300 hover:text-blue-700 transition-all shadow-sm"
                >
                    <Upload size={14} /> PESQUISA EM LOTE
                </button>
            </div>

            <div className="w-full max-w-[95rem] px-4 space-y-5 animate-fade-in pb-20">
                {buscaRealizada && resultados.length === 0 && !carregando && (
                    <div className="text-center p-10 bg-white rounded-2xl border-2 border-dashed border-slate-200">
                        <p className="text-slate-500 text-lg font-bold uppercase tracking-wider">Nenhum alvo encontrado no radar logístico.</p>
                    </div>
                )}

                {resultados.map((item, index) => (
                    <div key={index} className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1.5 h-full bg-blue-600"></div>

                        {/* CABEÇALHO */}
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pl-4">
                            <div>
                                <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">{item.pn}</h2>
                                <p className="text-slate-600 font-bold text-base mt-0.5">{item.nomenclatura}</p>
                                <div className="mt-2.5 flex flex-wrap gap-3">
                                    <span className={`text-sm font-mono font-black px-3 py-1 rounded-lg border ${item.nsn === 'Aguardando Cadastro' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                                        NSN/PI: {item.nsn}
                                    </span>
                                    {item.origem_nomenclatura ? (
                                        <span className="text-[11px] font-black px-3 py-1 rounded-lg border bg-blue-50 text-blue-700 border-blue-200 uppercase">
                                            Nome: {formatSource(item.origem_nomenclatura)}
                                        </span>
                                    ) : null}
                                    {item.origem_nsn ? (
                                        <span className="text-[11px] font-black px-3 py-1 rounded-lg border bg-slate-50 text-slate-600 border-slate-200 uppercase">
                            ...
                            <div className="mt-4 md:mt-0 flex flex-col items-end">
                                <span className={`py-2 px-6 rounded-xl text-xl font-black shadow-sm ${item.ppu_qtd > 0 ? 'bg-blue-600 text-white' : 'bg-slate-800 text-white'}`}>
                                    PPU: {item.ppu_qtd || 0} un
                                </span>
                                {item.ppu_qtd > 0 && <span className="text-[11px] font-black text-slate-500 mt-2 bg-slate-50 px-2 py-1 rounded border border-slate-200 uppercase">📍 Local: {item.ppu_locais}</span>}
                            </div>
                        </div>

                        {/* GRADE LOGÍSTICA DE 7 CARTÕES */}
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3.5 border-t border-slate-100 pt-5">
                            ...
                            <div className="bg-emerald-50/50 rounded-xl p-4 border border-emerald-100 flex flex-col shadow-sm">
                                ...
                                {item.oda && item.oda.length > 0 ? item.oda.map((oda, i) => (
                                    <div key={i} className="mb-3 last:mb-0 border-b border-emerald-200/40 pb-2 last:border-0">
                                        ...
                                        <p className="text-sm font-bold text-emerald-700 mt-0.5 uppercase">OC: {oda.oc_referencia}</p>
                            ...
                            <div className="bg-teal-50/50 rounded-xl p-4 border border-teal-100 flex flex-col shadow-sm">
                                ...
                                {item.odc && item.odc.length > 0 ? item.odc.map((o, i) => (
                                    <div key={i} className="mb-2 text-sm">
                                        <span className="font-black text-lg text-teal-900">{o.qtd_pendente} un</span> | <span className="text-sm font-mono font-bold text-teal-800">{o.pd_referencia}</span>
                                    </div>
                            ...
                            <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 flex flex-col shadow-sm">
                                ...
                                {item.foc && item.foc.length > 0 ? item.foc.map((f, i) => (
                                    <div key={i} className="text-sm font-bold text-blue-900"><span className="font-black text-lg">{f.qtd_pendente} un</span> | {f.documento_referencia}</div>
                                ...
                                {item.repairs && item.repairs.length > 0 ? (
                                    <div className="space-y-2 overflow-y-auto max-h-32 custom-scrollbar">
                                        {item.repairs.map((r, i) => (
                                            <div key={i} className="text-sm text-amber-900 border-b border-amber-200/40 pb-1.5">
                        ...
                        {((item.dicionario && item.dicionario.length > 0) || (item.alternativos && item.alternativos.length > 0) || (item.sb_referencias && item.sb_referencias.length > 0)) && (
                            <div className="bg-slate-50/80 rounded-xl p-5 border border-slate-200 mt-5 shadow-inner space-y-5">
                                <div className="flex justify-between items-center gap-3 flex-wrap border-b border-slate-200 pb-3">
                                    <span className="text-xs font-black text-slate-700 uppercase tracking-widest flex items-center gap-2">Referências Técnicas</span>
                                    <button onClick={() => { setAlvoAplicacoes(item.dicionario || []); setFontesMapa(item.fontes_alternativos || []); setModalAplicacoes(true); }} className="bg-blue-600 text-white text-[10px] font-black px-4 py-2 rounded-lg hover:bg-blue-700 transition-all">VER MAPA ({item.dicionario?.length || 0})</button>
                                </div>
                                                ...
                                                {item.alternativos.map((alt, idx) => (
                                                    <div key={idx} className="bg-white border border-slate-200 p-3 rounded-xl shadow-sm hover:border-amber-300 transition-all">
                                                        <div className="flex justify-between items-center mb-1 gap-2">
                                                            ...
                                                            <span className={`text-[10px] font-black px-2 py-0.5 rounded border whitespace-nowrap ${alt.ppu_qtd > 0 ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-50 text-slate-500'}`}>PPU: {alt.ppu_qtd}</span>
                                                        </div>
                                                        <span className="block text-[10px] font-bold text-slate-500">NSN: {alt.nsn || 'N/A'}</span>
                                                        <span className="block text-[10px] font-bold text-slate-500 mt-2">Fonte: {(alt.fonte && alt.fonte.length > 0) ? alt.fonte.join(' | ') : 'MANUAL TÉCNICO'}</span>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            {item.sb_referencias.map((sb, idx) => (
                                                <div key={`${sb.sb_numero}-${idx}`} className="bg-white border border-amber-200 p-4 rounded-xl shadow-sm">
                                                    <div className="flex items-start justify-between gap-3">
                                                        <div>
                                                            <p className="text-sm font-black text-slate-800 uppercase">{sb.sb_numero}</p>
            ...
            {modalAplicacoes && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden border-2 border-blue-600">
                        <div className="bg-slate-800 p-5 flex justify-between items-center border-b-4 border-blue-600">
                            <h3 className="text-lg font-black text-white uppercase tracking-wider flex items-center gap-3">Mapa de Instalação (DMC)</h3>
                            <button onClick={() => setModalAplicacoes(false)} className="text-slate-400 hover:text-red-400 transition-colors">
                        ...
                        <div className="p-6 overflow-y-auto flex-1 bg-slate-50 space-y-4">
                            {alvoAplicacoes.length > 0 ? alvoAplicacoes.map((app, i) => (
                                <div key={i} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-blue-300 transition-all">
                                    <div className="flex-1">
                                        <p className="text-base font-black text-slate-800 uppercase">📍 {app.techname || 'APLICAÇÃO GERAL'}</p>
                                        <p className="text-sm font-bold text-slate-500 mt-1 uppercase">{app.nomenclatura}</p>
                                    </div>
                                    <div className="flex gap-2 md:justify-end">
                                        <span className="bg-blue-50 text-blue-800 px-3 py-2 rounded-lg text-[10px] font-black border border-blue-200 flex flex-col items-center">
                                            DMC<span className="font-mono text-base mt-0.5">{app.dmc}</span>
                                        </span>
                                        <span className="bg-slate-100 text-slate-700 px-3 py-2 rounded-lg text-[10px] font-black border border-slate-200 flex flex-col items-center">
                                            ITEM<span className="font-mono text-base mt-0.5">{app.item_num}</span>
                                        </span>
                                        <span className="bg-slate-100 text-slate-700 px-3 py-2 rounded-lg text-[10px] font-black border border-slate-200 flex flex-col items-center">
                                            SUB<span className="font-mono text-base mt-0.5">{app.sub_item}</span>
                                        </span>
                                    </div>
                                </div>
                            )) : (
                                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
                                    <p className="text-base font-black text-slate-800 uppercase">Sem mapa técnico no manual</p>
                        ...
                        <div className="bg-slate-100 p-5 border-t border-slate-200 flex justify-end">
                            <button onClick={() => setModalAplicacoes(false)} className="bg-slate-800 text-white px-8 py-3 rounded-xl text-sm font-black hover:bg-slate-900 transition-all shadow-sm">FECHAR MAPA</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in">
                    <div className="w-full max-w-4xl bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden">
                        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between gap-4">
                            <div>
                                <h3 className="text-xl font-black text-slate-900 uppercase">Pesquisa em lote</h3>
                                <p className="text-sm font-bold text-slate-500">A planilha é usada só nesta consulta. Fechou o modal, limpou tudo.</p>
                            </div>
                            <button type="button" onClick={fecharModalLote} className="p-2 rounded-xl text-slate-500 hover:text-red-500 hover:bg-red-50 transition-all">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-5 max-h-[80vh] overflow-auto">
                            <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_auto] gap-3 items-end">
                                ...
                                <button type="button" onClick={executarPesquisaLote} disabled={loteCarregando || !arquivoLote} className="px-5 py-3 rounded-xl bg-blue-600 text-white font-black hover:bg-blue-700 disabled:opacity-50 inline-flex items-center justify-center gap-2">
                                    {loteCarregando ? <LoaderCircle size={16} className="animate-spin" /> : <Upload size={16} />} EXECUTAR
                                </button>
                                <button type="button" onClick={exportarPesquisaLote} disabled={loteExportando || !arquivoLote || !lotePreview} className="px-5 py-3 rounded-xl bg-emerald-600 text-white font-black hover:bg-emerald-700 disabled:opacity-50 inline-flex items-center justify-center gap-2">
                                    {loteExportando ? <LoaderCircle size={16} className="animate-spin" /> : <Download size={16} />} EXPORTAR EXCEL
                                </button>
                            </div>

                            {arquivoLote ? <p className="text-sm font-bold text-slate-500">Arquivo atual: {arquivoLote.name}</p> : null}
